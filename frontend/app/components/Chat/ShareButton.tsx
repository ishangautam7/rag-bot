...
export default function ShareButton({ sessionId, embedded = false }: ShareButtonProps) {
    ...
    const Content = (
        <div className={embedded ? "w-full" : "p-4"}>
            ...
            {viewMode === 'menu' && (
                <div className="space-y-2">
                    ...
                    {!isOwner && isCollaborative && (
                        <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <p className="text-sm font-medium text-[var(--color-foreground)]">You're a Collaborator</p>
                            </div>
                            <p className="text-xs text-[var(--color-foreground-muted)]">You can view and send messages in this chat. Only the owner can manage sharing settings.</p>
                            {members.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-blue-500/30">
                                    <p className="text-xs text-[var(--color-foreground-muted)] mb-2">{members.length} collaborators</p>
                                    <div className="flex -space-x-2">
                                        {members.slice(0, 5).map((member) => (
                                            <div key={member.id} className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-[10px] border-2 border-[var(--color-card)]" title={member.user.username || member.user.email}>
                                                {member.user.avatar ? (
                                        ...
                                        {members.length > 5 && (
                                            <div className="w-6 h-6 rounded-full bg-[var(--color-secondary)] flex items-center justify-center text-[var(--color-foreground)] text-[10px] border-2 border-[var(--color-card)]">
                                                +{members.length - 5}
                                            </div>
                                        )}
                                    </div>
                    ...
                    {isOwner && (
                        <>
                            ...
                            <div
                                ...
                                className="p-3 bg-[var(--color-background)] hover:bg-[var(--color-secondary)] border border-[var(--color-border)] rounded-lg cursor-pointer transition-colors"
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-[var(--color-foreground)]">View Only</p>
                                            <p className="text-xs text-[var(--color-foreground-muted)]">Anyone can read this chat</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {isShared && (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-[var(--color-foreground)]">Collaborate</p>
                                            <p className="text-xs text-[var(--color-foreground-muted)]">Invite others to chat together</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {isCollaborative && (
                    ...
                    {!isOwner && !isCollaborative && (
                        <div className="p-3 bg-[var(--color-secondary)] rounded-lg">
                            <p className="text-sm text-[var(--color-foreground-muted)]">You do not have permission to manage sharing for this chat.</p>
                        </div>
                    )}
                </div>
            )}
                ...
                <div>
                    {!isShared ? (
                        ...
                        <>
                            ...
                            <label className="text-xs text-[var(--color-foreground-muted)] mb-1 block">Share Link</label>
                            <div className="flex gap-2 mb-3">
                                <input
                                    type="text"
                                    value={readOnlyUrl}
                                    readOnly
                                    className="flex-1 bg-[var(--color-input)] border border-[var(--color-border)] rounded px-2 py-1.5 text-xs text-[var(--color-foreground)] truncate"
                                />
                                <button
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full px-3 py-2 text-sm text-red-500 hover:bg-red-500/10 rounded transition-colors flex items-center justify-center gap-2"
                            >
                ...
                <div>
                    {!isCollaborative ? (
                        ...
                        <>
                            ...
                            <label className="text-xs text-[var(--color-foreground-muted)] mb-1 block">Invite Link</label>
                            <div className="flex gap-2 mb-3">
                                <input
                                    type="text"
                            ...
                            {members.length > 0 && (
                                <div className="mb-3">
                                    <label className="text-xs text-[var(--color-foreground-muted)] mb-1 block">Collaborators ({members.length})</label>
                                    <div className="space-y-1 max-h-32 overflow-y-auto">
                                        {members.map((member) => (
                                            <div key={member.id} className="flex items-center justify-between p-1.5 bg-[var(--color-background)] rounded text-xs">
                                                <div className="flex items-center gap-2">
                                                ...
                                                {member.role !== 'OWNER' && (
                                                    <button
                                                        ...
                                                        className="p-0.5 text-[var(--color-foreground-muted)] hover:text-red-400 transition-colors"
                                                    >
        ...
        <div className="relative">
            <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${hasAnySharing
                    ...
                    : 'bg-[var(--color-secondary)] text-[var(--color-foreground-muted)] hover:text-[var(--color-foreground)] hover:bg-[var(--color-secondary)]'
                    }`}
                title="Share options"
            >
                {loading ? (
            ...
            {/* Dropdown */}
            {showDropdown && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-[var(--color-card-solid)] border border-[var(--color-border)] rounded-lg shadow-xl z-50">
                    {Content}
                </div>
            )}

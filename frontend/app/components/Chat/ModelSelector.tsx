...
export default function ModelSelector({ selectedModel, onModelChange, compact = true }: ModelSelectorProps) {
    ...
    const allModels: Model[] = [
        ...AVAILABLE_MODELS,
        ...customModels.map(cm => ({ id: cm.id, name: cm.name, provider: 'custom' as const }))
    ];
    ...
    const getProviderColor = (provider: string) => {
        switch (provider) {
            ...
            case 'custom': return 'bg-purple-400';
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            >
                <span className={`w-2 h-2 rounded-full ${getProviderColor(currentModel.provider)}`}></span>
            ...
            {isOpen && (
                <div className="absolute bottom-full left-0 mb-2 w-64 bg-neutral-900 border border-neutral-700 rounded-lg shadow-xl z-50 overflow-hidden">
                        ...
                        {allModels.map((model) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-neutral-800 transition-colors ${isSelected ? 'bg-emerald-500/10 border-l-2 border-emerald-500' : ''}`}
                                >
                                    ...
                                    {hasApiKey(model) ? (
                                        ...
                                        <span className="text-xs px-1.5 py-0.5 bg-neutral-700 text-neutral-400 rounded">No Key</span>
                                    )}
                                </button>
                            );
                        })}
